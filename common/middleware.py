...
class MoveJWTCookieIntoTheBody(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, *view_args, **view_kwargs):
        if request.path in [reverse("common:token_verify"), reverse("common:rest_logout")] and settings.JWT_AUTH_COOKIE in request.COOKIES:
            if request.body != b'':
...
class MoveJWTRefreshCookieIntoTheBody(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, *view_args, **view_kwargs):
        if request.path in [reverse("common:token_refresh"), reverse("common:rest_logout")] and settings.JWT_AUTH_REFRESH_COOKIE in request.COOKIES:
            if request.body != b'':
                data = json.loads(request.body)
